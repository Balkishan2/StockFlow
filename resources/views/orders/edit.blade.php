...
<div class="container-fluid py-4">
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-md-5">
            <h3 class="h3 mb-0 text-dark fw-bold">Edit Order #{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</h3>
        </div>
        <div class="col-md-7 d-flex justify-content-end align-items-center">
            ...
            <a href="{{ route('orders.view', $order->id) }}" class="btn btn-outline-secondary shadow-sm">
                <i class="fas fa-times fa-sm me-1"></i> Cancel
            </a>
        </div>
    </div>

    <form method="POST" action="{{ route('orders.edit', $order->id) }}" id="orderForm">
        @csrf
        <div class="row">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="customer_name" value="{{ $order->customer->name }}" placeholder="E.g. Acme Corp" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Email Address</label>
                                <input type="email" class="form-control" name="customer_email" value="{{ $order->customer->email }}" placeholder="contact@acme.com">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Phone</label>
                                <input type="text" class="form-control" name="customer_phone" value="{{ $order->customer->phone }}" placeholder="+1 555 000 0000">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Shipping Address</label>
                                <input type="text" class="form-control" name="shipping_address" value="{{ $order->customer->address }}" placeholder="123 Business Rd.">
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody id="itemsBody">
                                    @foreach($order->orderItems as $index => $item)
                                        @php
                                            ...
                                            $lineTax = $item->total - $lineTotal;
                                            $taxRate = $lineTotal > 0 ? ($lineTax / $lineTotal) * 100 : 0;
                                        @endphp
                                        <tr class="item-row">
                                            <td>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <input type="number" class="form-control invoice-input item-price px-1" name="items[{{ $index }}][price]" value="{{ number_format($item->unit_price, 2, '.', '') }}" step="0.01" required>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <input type="number" class="form-control invoice-input item-tax px-1" name="items[{{ $index }}][tax]" value="{{ number_format($taxRate, 1, '.', '') }}" min="0" step="0.1">
                                                    <span class="text-muted ms-1">%</span>
                                                </div>
                                            </td>
                                            <td class="text-end fw-bold row-total align-middle" style="color: #334155;">Rs0.00</td>
                                            <td class="text-center align-middle">
                                                <button type="button" class="remove-btn remove-item-btn p-1" {{ count($order->orderItems) == 1 ? 'disabled' : '' }}>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="summary-row border-bottom-0 pb-0">
                            ...
                            <div class="d-flex align-items-center" style="width: 100px;">
                                @php
                                    $calcTotal = $order->orderItems->sum('total');
                                    $discount = max(0, $calcTotal - $order->total_amount);
                                @endphp
                                <span class="text-muted me-2">Rs</span>
                                <input type="number" class="form-control invoice-input text-end px-1" id="summary-discount" name="discount" value="{{ number_format($discount, 2, '.', '') }}" step="0.01">
                            </div>
                        </div>
